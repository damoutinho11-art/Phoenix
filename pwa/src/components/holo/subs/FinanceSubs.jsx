...
const briefEur = value => {
  const n = Number(value)
  return Number.isFinite(n)
    ? n.toLocaleString('en-US', { style: 'currency', currency: 'EUR', minimumFractionDigits: 2 })
    : '—'
}
...
function formatRecommendationBrief(data = {}) {
  ...
  const week = data.week_label || 'CURRENT WEEK'
  const mode = String(data.portfolio_mode || 'portfolio').replace(/_/g, ' ').toUpperCase()
  const phase = data.phase_label ? ` · ${data.phase_label}` : ''
  const recLines = recs.length
    ? recs.map(r => {
      const asset = String(r.asset || 'asset').toUpperCase()
      const route = r.route || r.lane || 'route pending'
      const instrument = r.instrument?.name || r.instrument?.ticker || r.instrument?.symbol
      return `▸ ${asset} — ${briefEur(r.amount)} via ${route}${instrument ? ` · ${instrument}` : ''}`
    }).join('\n')
  ...
  return [
    ...
    '',
    'RATIONALE',
    data.rationale || 'No rationale returned by the recommendation engine.',
    '',
    'WATCH',
    warnings.length ? warnings.map(w => `▸ ${w}`).join('\n') : '▸ No engine warnings returned.',
    '',
    'SAFETY',
  ...
  return (
    <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            return (
              <button key={hd.name} onClick={() => onSel(i)} style={{ position: 'absolute', left: (50 + Math.cos(ang) * r).toFixed(1) + '%', top: (55 + Math.sin(ang) * r * 0.42).toFixed(1) + '%', transform: `translate(-50%,-50%) scale(${(0.72 + 0.55 * depth).toFixed(2)})`, opacity: (0.55 + 0.45 * depth).toFixed(2), background: 'none', border: 'none', cursor: 'pointer', textAlign: 'center', padding: 6, zIndex: 10 + Math.round(depth * 10) }}>
                <span style={{ display: 'block', animation: `${i % 2 ? 'holo-floatB' : 'holo-floatA'} 8s ease-in-out infinite` }}>
                  ...
                  <span style={{ display: 'block', ...financeMicro({ color: isSel ? W : a(ACC, '99'), whiteSpace: 'nowrap' }) }}>{hd.name}</span>
        ...
        <div style={{ flex: 1, minWidth: 264 }}>
          ...
          <div style={{ ...financeMicro({ letterSpacing: '.12em', color: a(ACC, '99') }), margin: '3px 0 12px' }}>{h.tk}</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', ...financeMicro({ letterSpacing: '.08em', color: a(ACC, '99') }), marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderTop: `1px solid ${a(ACC, '14')}` }}>
            <span style={financeMicro({ letterSpacing: '.12em', color: a(ACC, '99') })}>CURRENT WEIGHT</span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderTop: `1px solid ${a(ACC, '14')}`, borderBottom: `1px solid ${a(ACC, '14')}` }}>
            <span style={financeMicro({ letterSpacing: '.12em', color: a(ACC, '99') })}>DIRECTIVE</span>
            <span style={{ fontFamily: FM, fontSize: 10, letterSpacing: '.16em', color: c, border: `1px solid ${mix(c, 33)}`, padding: '4px 12px', textShadow: `0 0 8px ${c}` }}>{h.dir}</span>
          </div>
          <p style={{ margin: '12px 0 0', ...financeBody({ fontSize: '15.5px', lineHeight: 1.56, color: mix(BODY, 88) }) }}>{h.note}</p>
        </div>
    </div>
  )
}
            ...
            return (
              <button key={i} onClick={() => onToggle(i)} style={{ display: 'flex', gap: 12, alignItems: 'center', width: '100%', padding: '11px 13px', marginBottom: 9, background: deep(55), border: `1px solid ${on ? mix(G, 27) : a(ACC, '2a')}`, cursor: 'pointer', textAlign: 'left' }}>
                ...
                <span style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ display: 'block', ...financeMicro({ letterSpacing: '.08em', color: a(ACC, '99') }), marginTop: 2 }}>{ck.s}</span>
                </span>
                <span style={financeMicro({ letterSpacing: '.12em', color: on ? G : Y })}>{on ? 'VERIFIED' : 'PENDING'}</span>
              </button>
            )
          })}
        </div>
...
export function BriefContent() {
  ...
  const start = text => {
    const source = typeof text === 'string' ? text : briefText
    clearInterval(ivRef.current)
    setN(0)
    if (!source) return
    ivRef.current = setInterval(() => {
      setN(prev => {
        const next = Math.min(source.length, prev + 3)
        if (next >= source.length) clearInterval(ivRef.current)
        return next
      })
    }, 22)
  }
  ...
  if (error) {
    return (
      <div style={{ padding: '36px 18px', textAlign: 'center', border: `1px dashed ${a(ACC, '30')}`, background: deep(60) }}>
        <div style={{ fontFamily: FD, fontSize: 20, fontWeight: 700, color: ACC, marginBottom: 8 }}>UNABLE TO LOAD FINANCE BRIEF</div>
        <div style={{ maxWidth: 420, margin: '0 auto 16px', ...financeBody({ color: mix(BODY, 86) }) }}>
          Phoenix will not show the old fixture brief here. Reconnect the finance API and retry the live brief.
        </div>
        <button onClick={loadBrief} style={{ minHeight: 34, padding: '0 14px', ...financeButton({ fontWeight: 400, color: ACC }), background: deep(50), border: `1px solid ${a(ACC, '44')}`, cursor: 'pointer' }}>RETRY LIVE BRIEF</button>
      </div>
    )
  }

  const done = n >= briefText.length
  const pct = briefText.length ? Math.round((n / briefText.length) * 100) : 0
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <span style={financeMicro({ letterSpacing: '.14em', color: a(ACC, '99') })}>REAL BRIEF TRANSMISSION {pct}%</span>
        <button onClick={() => start()} style={{ minHeight: 30, padding: '0 14px', ...financeButton({ fontWeight: 400, color: a(ACC, 'cc') }), background: deep(50), border: `1px solid ${a(ACC, '44')}`, cursor: 'pointer' }}>↺ REPLAY</button>
      </div>
      ...
      {done && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center', marginTop: 12, animation: 'holo-fadeIn .5s ease both' }}>
          ...
          <span style={financeMicro({ letterSpacing: '.18em', color: a(ACC, 'cc') })}>END OF LIVE BRIEF</span>
