...
test('non-Home mobile domains switch to a dedicated command surface while desktop keeps HoloWings', async () => {
  ...
  assert.match(command, /<HoloCore domain=\{D\} hot=\{hot\} dimmed=\{!!focusPanel\} isShort=\{coreShort\} isMobile=\{isMobile\} sparks=\{atmosphere\.sparks\} showChips=\{showChips\} isHome=\{isHome\} \/>/)
...
test('mobile Home reserves separate vertical bands for the reactor readout and command composer', async () => {
  ...
  assert.match(core, /const coreTop = isMobileCommand[\s\S]*?: isMobileHome[\s\S]*?'27%'/)
  assert.match(core, /const coreSize = isMobileCommand[\s\S]*?: isMobileHome[\s\S]*?'min\(26vmin, 150px\)'/)
  assert.match(core, /const readoutTop = isMobileCommand[\s\S]*?: isMobileHome[\s\S]*?'min\(13vmin, 82px\)'/)
...
test('mobile command CSS locks the approved phone readability, spacing, and scrolling contract', async () => {
  ...
  assert.doesNotMatch(css, /\.holo-mobile-domain__actions\s*\{[\s\S]*?position:\s*sticky/)
  assert.match(css, /\.holo-mobile-panel\s*\{[^}]*min-width:\s*0/)
  assert.match(css, /\.holo-mobile-panel__body\s*\{[^}]*min-width:\s*0[^}]*overflow:\s*hidden/)
