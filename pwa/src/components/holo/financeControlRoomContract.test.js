...
test('finance hero value reads as a premium instrument number', async () => {
  ...
  assert.match(core, /const readoutTitleSize = isMobileCommand/)
  assert.match(core, /: isMoneyReadout \? 'clamp\(44px, 8\.2vmin, 72px\)' : isShort \? 'clamp\(34px, 6\.6vmin, 54px\)' : 'clamp\(50px, 9\.4vmin, 82px\)'/)
  assert.match(core, /\?\s*\(isMoneyReadout \? 'clamp\(28px, 8\.6vw, 38px\)' : 'clamp\(30px, 8\.8vw, 42px\)'\)/)
  assert.match(core, /fontSize:\s*readoutTitleSize/)
  assert.match(core, /transform:\s*'translateY\(-2px\)'/)
  assert.match(core, /const heroReadoutOffset = isShort \? 34 : 42/)
  assert.match(core, /const readoutTransform = isMobileCommand \? 'translate\(-50%, 0\)' : `translate\(-50%, \$\{heroReadoutOffset\}px\)`/)
  ...
  assert.match(core, /const readoutGap = isMobileCommand \? 6 : isHome \? 0 : 10/)
