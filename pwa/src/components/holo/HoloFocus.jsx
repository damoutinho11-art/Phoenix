  ...
  return (
    <>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '45%', transform: 'translate(-50%,-50%)', width: 'min(460px, calc(100vw - 28px))', zIndex: 86, animation: 'holo-focusIn .35s cubic-bezier(.2,.8,.4,1) both' }}>
        <div style={{ position: 'relative', overflow: 'hidden', textRendering: 'geometricPrecision', WebkitFontSmoothing: 'antialiased', MozOsxFontSmoothing: 'grayscale', border: `1px solid ${a(ACC, '55')}`, background: `linear-gradient(180deg, ${a(ACC, '1a')}, ${deep(90)})`, backdropFilter: 'blur(14px)', boxShadow: `0 0 60px ${a(ACC, '66')}, inset 0 0 40px ${a(ACC, '0d')}`, padding: '16px 18px 15px', clipPath: 'polygon(0 0, calc(100% - 18px) 0, 100% 18px, 100% 100%, 0 100%)' }}>
