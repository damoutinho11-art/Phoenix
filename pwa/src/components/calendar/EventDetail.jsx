    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#000', color: 'rgba(226,222,255,.94)', fontFamily: "'Saira Condensed',sans-serif" }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px 11px', borderBottom: `1px solid ${BORDER}`, position: 'sticky', top: 0, background: 'rgba(0,0,0,.96)', backdropFilter: 'blur(12px)', zIndex: 5, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ fontFamily: 'var(--display)', fontSize: 13, fontWeight: 700, letterSpacing: '.28em', color: VIOLET_BR, filter: 'drop-shadow(0 0 8px rgba(159,125,255,.25))' }}>EVENT DETAIL</span>
        </div>
        <span style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.14em', color: VIOLET, border: `1px solid rgba(159,125,255,.32)`, background: 'rgba(159,125,255,.055)', padding: '2px 8px' }}>{event?.badge || PROTO_EVENT.badge}</span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 88 }}>
        ...
        <div style={{ padding: '22px 20px 18px', borderBottom: `1px solid ${BORDER}`, background: 'linear-gradient(180deg,rgba(159,125,255,.045),transparent)' }}>
          ...
          <div style={{ fontFamily: 'var(--display)', fontSize: 36, fontWeight: 700, letterSpacing: '.08em', color: '#fff', lineHeight: 1, filter: 'drop-shadow(0 0 15px rgba(159,125,255,.28))' }}>
            {event ? event.title.toUpperCase() : PROTO_EVENT.title}
          </div>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.12em', color: TEXT_DIM, lineHeight: 1.7, marginTop: 9 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: `1px solid ${BORDER}` }}>
          {(event
            ...
            <div key={cell.label} style={{ padding: '14px 18px', borderRight: i % 2 === 0 ? `1px solid ${BORDER}` : 'none', borderBottom: i < 2 ? `1px solid ${BORDER}` : 'none' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.18em', color: MUTED, marginBottom: 5 }}>{cell.label}</div>
              <div style={{ fontFamily: 'var(--display)', fontSize: 18, fontWeight: 700, color: VIOLET_BR }}>{cell.val}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: '16px 18px', borderBottom: `1px solid ${BORDER}` }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED }}>PHOENIX PREP</span>
            <span style={{ fontFamily: 'var(--display)', fontSize: 16, fontWeight: 600, color: VIOLET }}>3 ITEMS</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {PROTO_EVENT.prep.map(item => (
              <div key={item.text} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', border: `1px solid rgba(32,216,236,.12)`, background: 'rgba(159,125,255,.025)' }}>
                ...
                <b style={{ fontFamily: 'var(--mono)', fontSize: 8, color: POS, letterSpacing: '.12em' }}>{item.status}</b>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: '16px 18px', borderBottom: `1px solid ${BORDER}` }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED }}>ACTIONS</span>
            <span style={{ fontFamily: 'var(--display)', fontSize: 16, fontWeight: 600, color: VIOLET }}>MANUAL</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {['MOVE', 'NOTES', 'REMIND'].map(lbl => (
              <div key={lbl} style={{ padding: '13px 0', border: `1px solid rgba(159,125,255,.28)`, background: 'rgba(159,125,255,.055)', fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.18em', color: VIOLET, textAlign: 'center', cursor: 'pointer' }}>{lbl}</div>
            ))}
            <div style={{ padding: '13px 0', background: VIOLET, border: 'none', fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.18em', color: '#080313', textAlign: 'center', boxShadow: '0 0 18px rgba(159,125,255,.28)', cursor: 'pointer' }}>DONE</div>
          </div>
        </div>
        ...
        <div style={{ margin: '14px 18px 32px', padding: '11px 13px', border: `1px solid rgba(32,216,236,.16)`, borderLeft: `3px solid ${VIOLET}`, background: 'rgba(159,125,255,.025)' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.2em', color: 'rgba(159,125,255,.52)', marginBottom: 6 }}>PHOENIX CONTEXT</div>
