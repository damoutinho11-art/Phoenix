...
function CountCard({ label, value }) {
  return (
    <div style={{ border: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.025)', padding: '10px 11px' }}>
      ...
      <div style={{ fontFamily: 'var(--phx-font-display)', fontSize: 24, color: LIME_BR, fontWeight: 700, marginTop: 3 }}>{value}</div>
    </div>
  )
}
    ...
    <div className="phx-scope-nutrition" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'radial-gradient(circle at 78% 4%, color-mix(in srgb, var(--phx-nutrition) 7%, transparent), transparent 34rem), linear-gradient(180deg, #081208 0%, var(--phx-bg) 42%, #04090e 100%)', color: 'rgba(220,248,236,.94)', fontFamily: 'var(--phx-font-body)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px 11px', borderBottom: `1px solid ${BORDER}`, position: 'sticky', top: 0, background: 'rgba(0,0,0,.96)', backdropFilter: 'blur(12px)', zIndex: 5, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
          ...
          <span style={{ fontFamily: 'var(--phx-font-display)', fontSize: 13, fontWeight: 700, letterSpacing: '.28em', color: LIME_BR }}>NUTRITION MEMORY</span>
        </div>
        <span style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.14em', color: LIME, border: `1px solid rgba(157,255,111,.32)`, background: 'rgba(157,255,111,.055)', padding: '2px 8px' }}>LOCAL · USER CONTROLLED</span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 130 }}>
        <div style={{ padding: '18px', borderBottom: `1px solid ${BORDER}`, background: 'linear-gradient(180deg,rgba(157,255,111,.045),transparent)' }}>
          <div style={{ fontFamily: 'var(--phx-font-display)', fontSize: 32, fontWeight: 700, letterSpacing: '.08em', color: '#fff' }}>PERSONAL FOOD BRAIN</div>
        ...
        <div style={{ margin: '16px 18px 0', padding: '13px', border: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.025)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
            <div>
              <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.2em', color: MUTED }}>REPEAT YESTERDAY</div>
        ...
        <div style={{ padding: '16px 18px 0' }}>
          <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED, marginBottom: 9 }}>PANTRY / PREFERRED STAPLES</div>
                ...
                <div key={staple.id} style={{ border: `1px solid rgba(255,209,102,.12)`, background: 'rgba(157,255,111,.018)', padding: '10px 11px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
                    <div>
                      ...
                      <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.1em', color: TEXT_DIM, marginTop: 2 }}>{fmt(staple.calories)} kcal · {fmt(staple.protein_g, 'g')} P · €{Number(staple.price_eur || 0).toFixed(2)}</div>
                    </div>
        ...
        <div style={{ padding: '16px 18px 0' }}>
          <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED, marginBottom: 9 }}>MANUAL AVOID</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8 }}>
            <input value={manualAvoid} onChange={e => setManualAvoid(e.target.value)} placeholder="e.g. mushrooms" style={{ minWidth: 0, background: 'rgba(0,0,0,.22)', border: `1px solid ${BORDER}`, color: '#fff', padding: '10px 11px', fontFamily: 'var(--phx-font-display)', fontSize: 15 }} />
        ...
        <div style={{ padding: '16px 18px' }}>
          <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED, marginBottom: 9 }}>ACTIVE MEMORY</div>
          <div style={{ border: `1px solid ${BORDER}`, background: 'rgba(0,0,0,.16)', padding: '0 13px' }}>
            {entries.length > 0 ? entries.map(entry => <EntryRow key={entry.id} entry={entry} onDelete={remove} />) : (
              <div style={{ padding: '15px 0', fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.13em', color: TEXT_DIM }}>NO MEMORY SAVED YET. MARK STAPLES, FAVORITES, OR AVOID FOODS ABOVE.</div>
            )}
          </div>
        </div>
      </div>
