  ...
  return (
    <NuScreen>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 88 }}>
        ...
        <div style={{ padding: '0 14px 16px', borderBottom: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 12 }}>
            {['recipes', 'staples', 'recent', 'custom'].map(t => (
              <button key={t} onClick={() => changeTab(t)} style={{ padding: '8px 0', background: t === tab ? 'rgba(157,255,111,.06)' : 'none', border: `1px solid ${t === tab ? 'rgba(157,255,111,.34)' : BORDER}`, color: t === tab ? LIME : MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 7.5, letterSpacing: '.12em', cursor: 'pointer' }}>
                {t.toUpperCase()}
              </button>
            ))}
          </div>
          ...
          {tab === 'custom' && (
            <div style={{ padding: '12px 14px', background: 'rgba(157,255,111,.025)', border: `1px solid rgba(157,255,111,.14)`, marginBottom: 12 }}>
              <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.16em', color: MUTED, marginBottom: 9 }}>CUSTOM FOOD · EXACT MACROS</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8 }}>
                {[
                  ...
                  <input key={field} type="number" min="0" value={custom[field]} onChange={e => setCustom(c => ({ ...c, [field]: e.target.value }))} placeholder={label} style={{ background: 'rgba(0,0,0,.22)', border: `1px solid ${BORDER}`, padding: '10px 12px', color: '#fff', fontSize: 13, fontFamily: 'var(--phx-font-mono)', outline: 'none', boxSizing: 'border-box' }} />
                ))}
              </div>
              {error && <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 11, color: '#ff5c7a', marginTop: 9 }}>{error}</div>}
              <button onClick={handleCustomLog} disabled={logging} style={{ width: '100%', marginTop: 12, padding: '14px 0', border: 'none', background: logging ? 'rgba(157,255,111,.3)' : LIME, color: '#001204', fontFamily: 'var(--phx-font-display)', fontSize: 15, fontWeight: 700, letterSpacing: '.22em', textAlign: 'center', boxShadow: `0 0 18px rgba(157,255,111,.28)`, cursor: 'pointer' }}>
            ...
            <div style={{ padding: '12px 14px', background: 'rgba(157,255,111,.04)', border: `1px solid rgba(157,255,111,.2)`, marginBottom: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                <div style={{ gridColumn: '1/-1', border: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.025)', padding: 11 }}>
                  <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.16em', color: MUTED, marginBottom: 7 }}>{selected.source === 'recent' ? 'REPEAT MEAL' : 'MEAL NAME'}</div>
                  <div style={{ fontFamily: 'var(--phx-font-display)', fontSize: 22, fontWeight: 700, color: '#fff' }}>{selected.name}</div>
                  {selected.serving && <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.12em', color: TEXT_DIM, marginTop: 4 }}>{selected.serving}{selected.is_batch_recipe && selected.full_calories ? ` · full batch ${Math.round(selected.full_calories)} kcal` : ''}</div>}
                </div>
                {[
                  ...
                  <div key={label} style={{ border: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.025)', padding: 11 }}>
                    <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.16em', color: MUTED, marginBottom: 7 }}>{label}</div>
                    <div style={{ fontFamily: 'var(--phx-font-display)', fontSize: 22, fontWeight: 700, color: '#fff' }}>{val}</div>
                  </div>
                ))}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 7, marginBottom: 12 }}>
                {[0.25, 0.5, 1, 1.5, 2].map(v => (
                  <button key={v} onClick={() => setServings(v)} style={{ padding: '8px 0', background: servings === v ? 'rgba(157,255,111,.08)' : 'none', border: `1px solid ${servings === v ? 'rgba(157,255,111,.34)' : BORDER}`, color: servings === v ? LIME : MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 8, cursor: 'pointer' }}>{v}×</button>
                ))}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <button onClick={() => adjustServings(-0.25)} style={{ width: 40, height: 40, background: 'none', border: `1px solid ${BORDER}`, color: LIME, fontSize: 20, cursor: 'pointer', fontFamily: 'var(--phx-font-display)', flexShrink: 0 }}>−</button>
                <span style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 18, color: '#fff', minWidth: 48, textAlign: 'center' }}>{servings}×</span>
                <button onClick={() => adjustServings(0.25)} style={{ width: 40, height: 40, background: 'none', border: `1px solid ${BORDER}`, color: LIME, fontSize: 20, cursor: 'pointer', fontFamily: 'var(--phx-font-display)', flexShrink: 0 }}>+</button>
                <span style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 9, color: MUTED }}>{selected.is_batch_recipe ? 'LOGGABLE SERVINGS' : 'SERVINGS'}</span>
              </div>
              {error && <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 11, color: '#ff5c7a', marginBottom: 8 }}>{error}</div>}
              <button onClick={handleLog} disabled={logging} style={{ width: '100%', padding: '15px 0', border: 'none', background: logging ? 'rgba(157,255,111,.3)' : LIME, color: '#001204', fontFamily: 'var(--phx-font-display)', fontSize: 15, fontWeight: 700, letterSpacing: '.22em', textAlign: 'center', boxShadow: `0 0 18px rgba(157,255,111,.28)`, cursor: 'pointer' }}>
          ...
          {tab !== 'custom' && (loading ? (
            <div style={{ padding: '24px 0', textAlign: 'center', color: MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 12 }}>Loading…</div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: '24px 0', textAlign: 'center', color: MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 12 }}>{tab === 'recent' ? 'No previous meals yet.' : 'Nothing found.'}</div>
          ) : filtered.map(item => {
            const isSel = selected?.id === (tab === 'recent' ? `recent-${item.id}` : item.id)
            return (
              <div key={`${tab}-${item.id}`} onClick={() => selectItem(item)} style={{ padding: '11px 12px', borderBottom: `1px solid rgba(157,255,111,.08)`, cursor: 'pointer', borderLeft: `3px solid ${isSel ? LIME : 'transparent'}`, background: isSel ? 'rgba(157,255,111,.04)' : 'none' }}>
                ...
                <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, color: TEXT_DIM, marginTop: 2 }}>
                ...
                {Array.isArray(item.tags) && item.tags.length > 0 && (
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 6 }}>
                    {item.tags.slice(0, 4).map(t => <span key={t} style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.1em', color: LIME, border: `1px solid rgba(157,255,111,.18)`, padding: '1px 5px', background: 'rgba(157,255,111,.025)' }}>{t}</span>)}
                  </div>
                )}
              </div>
            )
        ...
        <div style={{ margin: '14px 18px 32px', padding: '11px 13px', border: `1px solid rgba(157,255,111,.16)`, borderLeft: `3px solid ${LIME}`, background: 'rgba(157,255,111,.025)' }}>
