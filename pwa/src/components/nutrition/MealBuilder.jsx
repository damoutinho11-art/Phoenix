    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
      {MEAL_SLOTS.map(([value, label]) => (
        <button
          ...
          style={{
            border: `1px solid ${slot === value ? 'rgba(157,255,111,.55)' : 'rgba(157,255,111,.18)'}`,
...
function EditableItems({ items, onItems }) {
  ...
  return (
    <div style={{ marginTop: 12, borderTop: `1px solid rgba(157,255,111,.10)` }}>
      {items.map((item, index) => (
        <div key={`${item.item_id}-${item.name}-${index}`} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10, padding: '10px 0', borderBottom: `1px solid rgba(157,255,111,.08)` }}>
          <div>
  ...
  return (
    <div style={{ border: `1px solid rgba(157,255,111,.18)`, background: 'linear-gradient(180deg,rgba(157,255,111,.045),rgba(0,0,0,.1))', padding: 14 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
        ...
        <span style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.12em', color: CYAN, border: `1px solid rgba(157,255,111,.24)`, background: 'rgba(157,255,111,.045)', padding: '3px 7px' }}>EDITABLE</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 12 }}>
        <button onClick={resetItems} style={{ padding: '11px 0', border: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.035)', color: MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.16em', cursor: 'pointer' }}>RESET</button>
...
function TemplateCard({ template, onLog, onDelete, logging }) {
  ...
  return (
    <div style={{ border: `1px solid rgba(157,255,111,.14)`, background: 'rgba(157,255,111,.025)', padding: 12 }}>
  ...
  return (
    <div className="phx-scope-nutrition" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'radial-gradient(circle at 78% 4%, color-mix(in srgb, var(--phx-nutrition) 7%, transparent), transparent 34rem), linear-gradient(180deg, #081208 0%, var(--phx-bg) 42%, #04090e 100%)', color: 'rgba(220,248,236,.94)', fontFamily: 'var(--phx-font-body)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 120 }}>
        ...
        {data?.day_plan?.length > 0 && <div style={{ padding: '0 18px 16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
            {data.day_plan.map(step => (
              <div key={step.slot} style={{ border: `1px solid rgba(157,255,111,.14)`, background: 'rgba(157,255,111,.018)', padding: 11 }}>
        ...
        <div style={{ margin: '0 18px 16px', padding: '11px 13px', border: `1px solid rgba(157,255,111,.16)`, borderLeft: `3px solid ${LIME}`, background: 'rgba(157,255,111,.025)' }}>
