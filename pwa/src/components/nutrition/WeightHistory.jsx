...
function WeightTrendChart({ weights, baselineWeightKg }) {
  if (!weights || weights.length < 2) {
    return <div style={{ height: 132, display: 'flex', alignItems: 'center', justifyContent: 'center', color: MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 12, border: `1px solid rgba(157,255,111,.08)`, background: 'linear-gradient(rgba(157,255,111,.05) 1px,transparent 1px)', backgroundSize: '100% 25%' }}>Log at least 2 entries to see trend</div>
  }
  ...
  return (
    <div style={{ height: 132, position: 'relative', border: `1px solid rgba(157,255,111,.08)`, background: 'linear-gradient(rgba(157,255,111,.05) 1px,transparent 1px)', backgroundSize: '100% 25%' }}>
...
const HEAT_COLORS = {
  ...
  empty: { bg: 'rgba(157,255,111,.035)', border: 'rgba(157,255,111,.08)', color: MUTED },
}
