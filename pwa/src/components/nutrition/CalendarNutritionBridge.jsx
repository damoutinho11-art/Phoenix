...
const MUTED = 'rgba(157,255,111,.42)'
  ...
  return (
    <div className="phx-scope-nutrition" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'radial-gradient(circle at 78% 4%, color-mix(in srgb, var(--phx-nutrition) 7%, transparent), transparent 34rem), linear-gradient(180deg, #081208 0%, var(--phx-bg) 42%, #04090e 100%)', color: TEXT, fontFamily: 'var(--phx-font-body)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 92 }}>
        ...
        <div style={{ padding: '13px 18px', borderBottom: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.018)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, padding: '0 18px 14px', borderBottom: `1px solid ${BORDER}` }}>
          {[3, 7, 14].map(n => (
            <button key={n} onClick={() => setWindow(n)} style={{ flex: 1, padding: '10px 8px', border: `1px solid ${days === n ? 'rgba(157,255,111,.4)' : BORDER}`, background: days === n ? 'rgba(157,255,111,.08)' : 'rgba(157,255,111,.025)', color: days === n ? LIME_BR : MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.14em' }}>{n} DAYS</button>
          ))}
        </div>
        ...
        <div style={{ padding: '16px 18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {(data.days || []).map(day => (
              <div key={day.date} style={{ border: `1px solid ${BORDER}`, background: day.priority === 'high' ? 'rgba(255,92,122,.035)' : 'rgba(157,255,111,.02)', padding: '13px' }}>
