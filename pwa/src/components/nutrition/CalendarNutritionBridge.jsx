...
function Stat({ label, value }) {
  return (
    <div style={{ border: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.025)', padding: '11px 10px' }}>
      <div style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.18em', color: MUTED }}>{label}</div>
      <div style={{ fontFamily: 'var(--display)', fontSize: 24, lineHeight: 1, color: LIME_BR, fontWeight: 700, marginTop: 6 }}>{value}</div>
    </div>
  )
}
...
function PriorityPill({ priority }) {
  ...
  return <span style={{ border: `1px solid ${color}55`, color, padding: '2px 7px', fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.14em' }}>{String(priority || 'normal').toUpperCase()}</span>
}
...
export default function CalendarNutritionBridge({ onBack }) {
  ...
  if (loading) return <div style={{ height: '100%', background: '#000', color: MUTED, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--mono)', fontSize: 12 }}>Loading calendar nutrition bridge…</div>
  if (error || !data) return <div style={{ height: '100%', background: '#000', color: TEXT, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' }}>{error || 'No calendar bridge data.'}</div>
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#000', color: TEXT, fontFamily: "'Saira Condensed',sans-serif" }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px 11px', borderBottom: `1px solid ${BORDER}`, position: 'sticky', top: 0, background: 'rgba(0,0,0,.96)', zIndex: 5 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ fontFamily: 'var(--display)', fontSize: 13, fontWeight: 700, letterSpacing: '.24em', color: LIME_BR }}>CALENDAR NUTRITION</span>
        </div>
        <span style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.14em', color: LIME, border: `1px solid rgba(157,255,111,.32)`, padding: '2px 8px' }}>V2.3 · IMPORT</span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 92 }}>
        <div style={{ padding: '18px', borderBottom: `1px solid ${BORDER}`, background: 'linear-gradient(180deg,rgba(157,255,111,.045),transparent)' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED, marginBottom: 8 }}>BRIDGE SUMMARY</div>
          <div style={{ fontFamily: 'var(--display)', fontSize: 24, fontWeight: 700, color: '#fff', lineHeight: 1.15 }}>{data.summary}</div>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.12em', color: DIM, marginTop: 10 }}>{(data.calendar_source?.active_source || 'fixture').toUpperCase().replaceAll('_', ' ')} · NO AI REQUIRED · NO AUTO LOGGING</div>
        </div>

        <div style={{ padding: '13px 18px', borderBottom: `1px solid ${BORDER}`, background: 'rgba(32,216,236,.018)' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED, marginBottom: 7 }}>PLAAN SOURCE BOUNDARY</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <div style={{ border: `1px solid ${BORDER}`, padding: '10px', background: 'rgba(0,0,0,.18)' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.16em', color: MUTED }}>SOURCE</div>
              <div style={{ fontFamily: 'var(--display)', fontSize: 17, fontWeight: 700, color: LIME_BR, marginTop: 4 }}>{(data.calendar_source?.active_source || 'fixture').replaceAll('_', ' ').toUpperCase()}</div>
            </div>
            <div style={{ border: `1px solid ${BORDER}`, padding: '10px', background: 'rgba(0,0,0,.18)' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.16em', color: MUTED }}>LIVE FETCH</div>
              <div style={{ fontFamily: 'var(--display)', fontSize: 17, fontWeight: 700, color: data.calendar_source?.live_enabled ? '#ffd56b' : CYAN, marginTop: 4 }}>{data.calendar_source?.live_enabled ? 'MANUAL ON' : 'OFF BY DEFAULT'}</div>
            </div>
          </div>
        ...
        <div style={{ padding: '14px 18px', borderBottom: `1px solid ${BORDER}`, background: 'rgba(157,255,111,.018)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
            <div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED }}>MANUAL PLAAN SNAPSHOT IMPORT</div>
            ...
            <span style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.14em', color: LIME, border: `1px solid rgba(157,255,111,.22)`, padding: '3px 7px' }}>V2.3 · SAFE IMPORT</span>
          </div>
          {latestImport?.configured && <div style={{ border: `1px solid rgba(157,255,111,.14)`, background: 'rgba(157,255,111,.03)', padding: '8px 10px', marginBottom: 9, fontFamily: 'var(--mono)', fontSize: 7.5, letterSpacing: '.09em', color: LIME_BR }}>
        ...
        <div style={{ display: 'flex', gap: 8, padding: '0 18px 14px', borderBottom: `1px solid ${BORDER}` }}>
          {[3, 7, 14].map(n => (
            <button key={n} onClick={() => setWindow(n)} style={{ flex: 1, padding: '10px 8px', border: `1px solid ${days === n ? 'rgba(157,255,111,.4)' : BORDER}`, background: days === n ? 'rgba(157,255,111,.08)' : 'rgba(32,216,236,.025)', color: days === n ? LIME_BR : MUTED, fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.14em' }}>{n} DAYS</button>
          ))}
        </div>
        ...
        <div style={{ padding: '16px 18px', borderBottom: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7 }}>
            {['read_only_calendar', 'no_plaan_mutations', 'no_credentials_in_nutrition', 'no_raw_page_sent_to_ai', 'no_auto_logging', 'no_auto_shopping'].map(key => (
              <div key={key} style={{ padding: '9px 10px', border: `1px solid rgba(157,255,111,.14)`, background: 'rgba(157,255,111,.025)', fontFamily: 'var(--mono)', fontSize: 7.5, letterSpacing: '.1em', color: data.safety?.[key] ? LIME : '#ff5c7a' }}>◆ {key.replaceAll('_', ' ').toUpperCase()}</div>
            ))}
          </div>
        </div>

        <div style={{ padding: '16px 18px' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.22em', color: MUTED, marginBottom: 10 }}>SCHEDULE-AWARE TIMING</div>
            ...
            {(data.days || []).map(day => (
              <div key={day.date} style={{ border: `1px solid ${BORDER}`, background: day.priority === 'high' ? 'rgba(255,92,122,.035)' : 'rgba(32,216,236,.02)', padding: '13px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div>
                    <div style={{ fontFamily: 'var(--display)', fontSize: 18, fontWeight: 700, color: '#fff', letterSpacing: '.08em' }}>{day.date}</div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.14em', color: DIM, marginTop: 2 }}>{day.day_type?.replaceAll('_', ' ').toUpperCase()} · {day.is_training_day ? 'TRAINING' : 'REST'}</div>
                  </div>
                  <PriorityPill priority={day.priority} />
                </div>

                {day.events?.length > 0 && <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 9 }}>
                  {day.events.map(ev => <div key={ev.event_id} style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.08em', color: TEXT }}>● {ev.time_label} · {ev.event_type.toUpperCase()} · {ev.title}</div>)}
                </div>}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
                  {(day.planner_adjustments || []).map(adj => <span key={adj} style={{ fontFamily: 'var(--mono)', fontSize: 7, letterSpacing: '.11em', color: LIME, border: `1px solid rgba(157,255,111,.18)`, padding: '3px 6px' }}>{adj.replaceAll('_', ' ').toUpperCase()}</span>)}
                </div>
              </div>
            ))}
          </div>
        ...
        {(data.fetch_warnings || []).length > 0 && <div style={{ margin: '0 18px 18px', padding: '12px', border: `1px solid rgba(255,213,107,.18)`, background: 'rgba(255,213,107,.035)' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 8, letterSpacing: '.2em', color: '#ffd56b', marginBottom: 6 }}>CALENDAR SNAPSHOT WARNINGS</div>
