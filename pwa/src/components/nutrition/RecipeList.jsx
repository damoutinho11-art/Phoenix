    ...
    <div className="phx-scope-nutrition" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'radial-gradient(circle at 78% 4%, color-mix(in srgb, var(--phx-nutrition) 7%, transparent), transparent 34rem), linear-gradient(180deg, #081208 0%, var(--phx-bg) 42%, #04090e 100%)', color: 'rgba(220,248,236,.94)', fontFamily: 'var(--phx-font-body)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px 11px', borderBottom: `1px solid ${BORDER}`, position: 'sticky', top: 0, background: 'rgba(0,0,0,.96)', backdropFilter: 'blur(12px)', zIndex: 5, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span onClick={onBack} style={{ color: '#9dff6f', fontSize: 16, marginRight: 10, cursor: 'pointer' }}>←</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 88 }}>
        <div style={{ padding: '18px', borderBottom: `1px solid ${BORDER}`, background: 'linear-gradient(180deg,rgba(157,255,111,.04),transparent)' }}>
          <div style={{ fontFamily: 'var(--phx-font-display)', fontSize: 32, fontWeight: 700, letterSpacing: '.08em', color: '#fff', filter: 'drop-shadow(0 0 14px rgba(157,255,111,.28))' }}>NEXT MEAL OPTIONS</div>
          <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.12em', color: TEXT_DIM, lineHeight: 1.7, marginTop: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 6, overflowX: 'auto', padding: '10px 18px', borderBottom: `1px solid ${BORDER}` }}>
          ...
          {DISABLED_FILTERS.map(f => (
            <button key={f} disabled title="Recipe metadata not available yet" style={{ flexShrink: 0, fontFamily: 'var(--phx-font-mono)', fontSize: 8, letterSpacing: '.16em', border: `1px solid rgba(157,255,111,.08)`, padding: '7px 10px', background: 'none', color: 'rgba(157,255,111,.18)', cursor: 'not-allowed' }}>{f}</button>
          ))}
        </div>

        <div>
          {loading ? (
            <div style={{ padding: '40px', textAlign: 'center', color: MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 12 }}>Loading recipes…</div>
          ) : error ? (
            <div style={{ padding: '40px 24px', textAlign: 'center', color: MUTED, fontFamily: 'var(--phx-font-mono)', fontSize: 11, lineHeight: 1.7 }}>{error}</div>
            ...
            <div key={r.id} onClick={() => setExpanded(expanded === r.id ? null : r.id)} style={{ padding: '14px 16px', borderBottom: `1px solid rgba(157,255,111,.08)`, display: 'grid', gridTemplateColumns: '1fr 72px', gap: 12, cursor: 'pointer' }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 8 }}>
                  {r.tags.map(t => <span key={t} style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, letterSpacing: '.1em', color: LIME, border: `1px solid rgba(157,255,111,.22)`, padding: '2px 6px', background: 'rgba(157,255,111,.035)' }}>{t}</span>)}
                </div>
                {expanded === r.id && (
                  <div style={{ display: 'flex', gap: 16, marginTop: 12 }}>
                    ...
                    ].map(({ label, val }) => <div key={label}><div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 9, color: MUTED, letterSpacing: '.1em', marginBottom: 2 }}>{label}</div><div style={{ fontFamily: 'var(--phx-font-display)', fontSize: 14, color: '#fff' }}>{val}</div></div>)}
                  </div>
                )}
              </div>
              <div>
                <div style={{ fontFamily: 'var(--phx-font-display)', fontSize: 24, fontWeight: 700, color: LIME_BR, textAlign: 'right', lineHeight: 1 }}>{Math.round(r.kcal)}</div>
                <div style={{ fontFamily: 'var(--phx-font-mono)', fontSize: 7, color: MUTED, textAlign: 'right', marginTop: 4 }}>KCAL</div>
              </div>
            </div>
          ))}
        </div>
